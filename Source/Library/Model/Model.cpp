...
namespace library
{
    ...
    HRESULT Model::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
    {
        ...
        if (m_pScene)
        {
            ...
            XMMATRIX globalTransform = ConvertMatrix(m_pScene->mRootNode->mTransformation);
            XMVECTOR det = XMMatrixDeterminant(globalTransform);
            m_globalInverseTransform = XMMatrixInverse(&det, globalTransform);
        ...
        // create vertex buffer, m_animationBuffer with initial data m_AnimationData
        D3D11_BUFFER_DESC descAnimationData = {
        ...
        hr = pDevice->CreateBuffer(&descAnimationData, &animationInitData, m_animationBuffer.GetAddressOf());
        ...
        D3D11_BUFFER_DESC descSkinningConstant = {
        ...
        hr = pDevice->CreateBuffer(&descSkinningConstant, nullptr, m_skinningConstantBuffer.GetAddressOf());
    ...
    void Model::Update(_In_ FLOAT deltaTime)
    {
        ...
        if (m_pScene->HasAnimations())
        {
            ...
            {
                // calculate the bone transform matrices by calling readNodeHierarchy, starting with root node
                readNodeHierarchy(animationTimeTicks, m_pScene->mRootNode, identity);
                // resize m_aTransform same as m_aBoneInfo
    ...
    void Model::readNodeHierarchy(_In_ FLOAT animationTimeTicks, _In_ const aiNode* pNode, _In_ const XMMATRIX& parentTransform)
    {
        ...
        const aiNodeAnim* pNodeAnim = findNodeAnimOrNull(m_pScene->mAnimations[0], pNode->mName.data);
        
        if (pNodeAnim)
        {
            XMFLOAT3 outputScaling = XMFLOAT3();
            interpolateScaling(outputScaling, animationTimeTicks, pNodeAnim);
            XMMATRIX scalingMatrix = XMMatrixScaling(outputScaling.x, outputScaling.y, outputScaling.z);

            XMFLOAT3 outputTranslation = XMFLOAT3();
            interpolatePosition(outputTranslation, animationTimeTicks, pNodeAnim);
            XMMATRIX translationMatrix = XMMatrixTranslation(outputTranslation.x, outputTranslation.y, outputTranslation.z);

            XMVECTOR outputRotation = XMVECTOR();
            interpolateRotation(outputRotation, animationTimeTicks, pNodeAnim);
            XMMATRIX rotationMatrix = XMMatrixRotationQuaternion(outputRotation);
        ...
        std::unordered_map<std::string, UINT>::const_iterator nodeKeyIdx = m_boneNameToIndexMap.find(pNode->mName.data);
        if (nodeKeyIdx != m_boneNameToIndexMap.end())
        {

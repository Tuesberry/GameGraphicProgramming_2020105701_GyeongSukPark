...
namespace library
{
	...
	HRESULT RenderTexture::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
	{
		...
		// create texture2D m_texture2D used as the shadow map
		...
		hr = pDevice->CreateTexture2D(&textureDesc, nullptr, m_texture2D.GetAddressOf());
		if (FAILED(hr))
		{
			return hr;
		}

		// create render target view m_renderTargetView
		...
		// create shader resource view m_shaderResourceView
		...
		// create sampler state m_samplerClamp with D3D11_Texture_ADDRESS_CLAMP

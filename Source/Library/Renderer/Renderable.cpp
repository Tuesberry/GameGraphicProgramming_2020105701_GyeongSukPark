...
namespace library
{
    ...
    )
    {
        ...
        // if m_aNormalData is empty
        if (m_aNormalData.empty())
        {
            // call calculateNormalMapVectors
            calculateNormalMapVectors();
        }

        // create m_normalBuffer vertex buffer
        D3D11_BUFFER_DESC normalDesc = {
            .ByteWidth = sizeof(NormalData) * static_cast<UINT>(m_aNormalData.size()),
            .Usage = D3D11_USAGE_DEFAULT,
            .BindFlags = D3D11_BIND_VERTEX_BUFFER,
            .CPUAccessFlags = 0,
            .MiscFlags = 0
        ...
        hr = pDevice->CreateBuffer(&normalDesc, &normalData, m_normalBuffer.GetAddressOf());
        if (FAILED(hr))
        {
            return hr;
        }
    ...
    void Renderable::calculateNormalMapVectors()
    {
        ...
        {
            // calculate tangent/bitangent vectors of vertices in the face with calculateTangentBitangent
            calculateTangentBitangent(aVertices[aIndices[i * 3]], aVertices[aIndices[i * 3 + 1]], aVertices[aIndices[i * 3 + 2]], tangent, bitangent);

            // store tangent/bitangent vectors of each vertex in m_aNormalData
